# Reader context

Profile for librarians, recommenders, and curriculum builders. This is **not** a book list — it explains education, work, taste, and what not to recommend.  
**Gaps and ordered study paths:** [[Gaps and Study Paths]].

## Education

| Degree | Field |
|--------|--------|
| Master of Science | Applied Physics |
| Master of Business Administration | MBA |
| Bachelor | Psychology |
| Bachelor | Economic History |

### What “covered by degree” means here

Assume **undergraduate/graduate foundations** are already in place for those fields. Prefer leisure, narrative, advanced, or cross-disciplinary recommendations over intro textbooks or survey primers.

| Field | Treat as already known (do not “gap-fill” with intro material) |
|-------|------------------------------------------------------------------|
| **Applied physics** | Classical mechanics, E&M, thermo/stat mech, quantum intro, relativity basics, mathematical methods, lab/scientific method. Popular *intro* physics/cosmos books are usually redundant unless they are exceptional writing or a new research frontier. |
| **MBA** | Strategy, finance basics, accounting literacy, org behavior, marketing, operations — at MBA survey level. Prefer deeper or narrative works (crises, industries, biographies of practice) over “MBA in one book” titles. |
| **Psychology** | Intro psych, research methods, core clinical/cognitive/social/developmental concepts at bachelor level. Memoirs and advanced monographs still useful; Psych 101 is not. Pop dual-process catalogs (e.g. Kahneman airport form) are a bad fit — see DNF. |
| **Economic history** | Long-run growth, industrialization, crises, institutions, and historical political economy at bachelor level. Still room for great narrative histories and new research; skip “what is the Industrial Revolution?” primers. |

## Work & AI practice (not a leisure gap)

| Fact | Implication for recommendations |
|------|----------------------------------|
| Uses AI **daily** (including coding agents in parallel) | No “get started with AI” books |
| Works with AI adoption at a **senior level** | No exec transformation playbooks or change-management cheerleading |
| Builds and deploys **AI systems and agents** hands-on | No tooling/platform primers |
| **Patented inventor**, mostly AI-related | Assume expert practitioner; never recommend as if under-skilled |
| Societal / civilizational **implications** of AI can still interest | Only if **non-basic** and non-flattering; see [[Gaps and Study Paths]] Path C |

## Intellectual taste (hard filter)

**Need to be challenged.** Prefer material that makes the reader feel behind, wrong, or intellectually “stupid” in the productive sense — not soothed.

| Attracts | Repels (instant disqualify) |
|----------|-------------------------------|
| Dense argument, adversarial frames, primary weight | Consensus cheering; “we in this room are the clear-sighted ones” |
| Books that risk being disliked | TED/exec-keynote energy; audience flattery |
| Dark or structural implications | Hopeium, status-congruent “smart person” brand books |
| Feeling talked *at* by a superior mind | Being congratulated for showing up |
| **First principles** (power, knowledge, value, constraint, credit, form) | **Topic-first study** (climate, AI, “current thing”) as the *start* of a path |

**Study preference (binding):** Good study starts from **first principles**, not applied topics. Climate and AI are **not** first principles — they are applications of energy, technique, power, capital. Core paths in [[Gaps and Study Paths]]: **P, K, M, T, $, L, H**. Path **H** = one deep historical spine (e.g. Rome, Cold War), not world-survey tourism. App-Climate / App-AI only after core (default skip App-AI).

**Anecdote (binding):** At an executive-track AI conference, a speaker praising the audience for being there and clear-sighted was a **total turnoff**. That register of writing/talking is rejected in recommendations — especially on AI.

**Heuristic for agents:** If a book would open a conference track for AI leaders with warmth and moral self-congratulation, do not recommend it. Do not lead with climate or AI topic books.

## Reading-log caveats

1. **Vault start:** Leisure reading systematically logged from the Books Google Calendar **2005 onward**. Earlier books (especially **1999–2005 studies**) may be missing or only backfilled when remembered.
2. **Degree reading ≠ vault entries.** Course books and academic reading were not logged as calendar “books finished.”
3. **Work expertise ≠ vault entries.** AI practice is professional, not a shelf of finished AI books.
4. **“Gap” in a MOC** often means *not logged as leisure reading*, not *never encountered*.
5. **Exact dates** for retrospective backfills may be approximate (`date_precision: approximate` on the note).

## Series confirmed complete

- **Mars trilogy** (Kim Stanley Robinson): [[Red Mars]], [[Green Mars]], [[Blue Mars]] — all finished (Green/Blue dated 2006; Red backfilled as pre-Green, date approximate).

## Pre-library / studies-era leisure books (backfilled)

| Work | Notes |
|------|--------|
| [[Nineteen Eighty-Four]] (Orwell) | Read during studies, ~1999–2005; long before the calendar library. |
| [[Relativity]] (Einstein) | Short popular exposition (*Relativity: The Special and the General Theory*); physics background. |
| [[The Trial]] (Kafka) | High school; procedure-as-nightmare angle on Path L already closed. |
| [[Röda rummet]] (Strindberg) | Confirmed read; Swedish classic (*The Red Room*). |

## Active / paused (not finished)

- **[[This Changes Everything]]** — Naomi Klein (*Capitalism vs. the Climate*). **Status: reading** (2026); not finished. Related finished Klein: [[The Shock Doctrine]] (2018).

## Did not finish (DNF) — do not re-recommend as gap-fills

- **[[Thinking, Fast and Slow]]** (Kahneman) — started years ago; stopped by ~ch. 1; felt “stupid” / uncompelling. With a **bachelor in Psychology**, the pop dual-process framing is a poor leisure fit. See [[Gaps and Study Paths]] Path **K** for non-Kahneman alternatives only if the theme still matters.
- **[[The Downing Street Years]]** (Thatcher) — nearly finished (~p. 540); **enjoyed a lot** until it felt repetitive. Strong partial read — not “never touched Thatcher.”
- **[[The Confessions]]** (Rousseau) — roughly halfway; became slow. Partial classic exposure only.

## How agents should use this

1. Read **this file** + **[[Gaps and Study Paths]]** before recommending.
2. **Librarian:** Cross-check degrees, AI practice, and DNF before “never read X.”
3. **Recommender:** Apply taste filter (challenge > cheerleading); prefer **core first-principles paths** over topic apps; never flatter; never lead with climate/AI.
4. **Curriculum builder:** Build on degree + work foundations; fill *vault leisure* gaps that still bite.
